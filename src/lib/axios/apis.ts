// AUTH APIS
export const LOGIN_API = `/auth/login`;
export const SOCIAL_LOGIN_API = `/auth/social-login`;
export const REGISTER_API = `/auth/signup`;
export const FORGOT_PASSWORD_API = `/auth/forgot-password/request-otp`;
export const SEND_VERIFICATION_LINK_API = '/auth/send-verification-link';
export const VERIFY_TOKEN_API = `/auth/verify-token`;
export const VERIFY_REGISTRATION_TOKEN_API = `/auth/verify-registration-token`;
export const RESET_PASSWORD_API = `/auth/forgot-password/verify-otp`;
export const LOGOUT_API = `/auth/logout`;
