...
export function useStoryTableFilters() {
  ...
  const [pageSize, setPageSize] = useQueryState(
    'limit',
    parseAsInteger.withOptions({ shallow: false }).withDefault(50)
  );
  ...
  const handleColumnSort = useCallback((columnId: string, direction: 'asc' | 'desc' | false) => {
    if (direction === false) {
      // Clear sorting
      setSortField('');
      setSortOrder(1);
  ...
  const resetFilters = useCallback(() => {
    ...
    setSortOrder(1);
    setPage(1);
  }, [setSearchQuery, setCategoryIds, setSortField, setSortOrder, setPage]);
