...
import { useDeleteCity } from '@/hooks/content/use-city';
...
export function CellAction({ data }: CellActionProps) {
  ...
  const onCopy = (id: string) => {
    ...
    toast.success('City ID copied to clipboard.');
  };
    ...
    <>
      <DropdownMenu open={dropdownOpen} onOpenChange={setDropdownOpen}>
        ...
        <DropdownMenuContent align="end">
          ...
          <DropdownMenuItem onClick={() => onCopy(`${process.env.NEXT_PUBLIC_WEB_BASE_URL}/city/${data._id}`)}>
            <Copy className="mr-2 h-4 w-4" /> Copy Unique Link
          </DropdownMenuItem>
          <DropdownMenuItem
            onClick={() => router.push(`/dashboard/content/city/${data._id}`)}
          >
